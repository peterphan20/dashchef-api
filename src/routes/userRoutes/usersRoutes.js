module.exports = async function usersRoutes(fastify) {
	fastify.get("/user/:id", async function (request) {
		...
		const { rows } = await client.query(
			'SELECT id, first_name AS "firstName", last_name AS "lastName", email, address, phone, signup_date AS "signupDate", avatar_url AS "avatarURL" FROM users WHERE id=$1',
			[id]
		);
		client.release();
	...
	fastify.get("/validateToken", async (request) => {
